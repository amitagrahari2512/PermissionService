...
public class InMemoryUserDetailsService implements UserDetailsService, UserService {
	...
	private void init() {
		...
		users.put("admin", new ProjectSecurityUser("admin", "{noop}password", UserRole.ADMIN));
		users.put("pm1", new ProjectSecurityUser("pm1", "{noop}password", UserRole.PM));
		users.put("pm2", new ProjectSecurityUser("pm2", "{noop}password", UserRole.PM));
		users.put("dev1", new ProjectSecurityUser("dev1", "{noop}password", UserRole.DEVELOPER));
		users.put("dev2", new ProjectSecurityUser("dev2", "{noop}password", UserRole.DEVELOPER));
		users.put("test1", new ProjectSecurityUser("test1", "{noop}password", UserRole.TESTER));
		users.put("test2",new ProjectSecurityUser("test2", "{noop}password", UserRole.TESTER));
	}
